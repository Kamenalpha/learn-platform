...
import com.rag.edu.service.ExamService;
...
public class ExamController {
    ...
    public Result<Map<String, Object>> generate(@RequestBody GenerateReq req) {
        return Result.ok(examService.generate(UserContext.userId(), req));
    }

    @GetMapping("/paper/{paperId}")
    public Result<Map<String, Object>> paperDetail(@PathVariable Long paperId) {
        return Result.ok(examService.paperDetail(paperId));
    }

    @PostMapping("/submit")
    public Result<Map<String, Object>> submit(@RequestBody GradeReq req) {
        return Result.ok(examService.grade(UserContext.userId(), req));
    }

    @GetMapping("/exams")
    public Result<List<Map<String, Object>>> exams() {
        return Result.ok(examService.listExams(UserContext.userId()));
    }

    @GetMapping("/mistakes")
    public Result<List<Map<String, Object>>> mistakes() {
        return Result.ok(examService.mistakes(UserContext.userId()));
    }

    @PostMapping("/mistakes/{mistakeId}/master")
    public Result<Void> master(@PathVariable Long mistakeId) {
        examService.masterMistake(UserContext.userId(), mistakeId);
        return Result.ok();
    }
}
